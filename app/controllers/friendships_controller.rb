class FriendshipsController < ApplicationController
  ...
  def create
    ...
    @friendship = current_user.sent_requests.build(receiver_id: params[:user_id])
    if @friendship.save
  ...
  def accept_friend
    @friendship = Friendship.find_by(requestor_id: params[:user_id], receiver_id: current_user.id, status: false)
    return unless @friendship # return if no record is found

    @friendship.status = true
    if @friendship.save
      flash[:success] = 'Friend Request Accepted!'
      @friendship2 = current_user.sent_requests.build(receiver_id: params[:user_id], status: true)
      @friendship2.save
    else
  ...
  def decline_friend
    @friendship = Friendship.find_by(requester_id: params[:user_id], receiver_id: current_user.id, status: false)
    return unless @friendship # return if no record is found
